...
//   return (
//     <header className="bg-bar-color text-white  z-20 flex justify-between items-center fixed w-full top-0">
...
//         {/* Hamburger Button */}
//         <button
//           className="md:hidden cursor-pointerfixed right-4 flex justify-center items-center w-10 h-10 bg-[#001220] rounded z-50 shadow-lg"
...
export default function Header() {
  ...
  return (
    <header className="bg-bar-color text-white z-100 fixed w-full top-0">
      <div className="w-full max-w-7xl mx-auto px-4 py-4 flex items-center justify-between relative z-0">
        {/* Logo */}
        <a href="#home">
          <img
            src={logo}
        ...
        <nav className="hidden md:flex space-x-6">
          {links.map(({ href, label }) => (
            <a
              ...
              href={href}
              onClick={() => setMenuOpen(false)}
              className="relative group transition duration-200 text-white"
            >
              {label}
              <span className="absolute left-0 -bottom-1 w-0 h-0.5 bg-white transition-all duration-300 group-hover:w-full"></span>
            </a>
        ...
        {/* Hamburger Button */}
        <button
          className="md:hidden flex justify-center items-center w-10 h-10 bg-[#001220] rounded z-50 shadow-lg"
          onClick={() => setMenuOpen(!menuOpen)}
          aria-label="Toggle menu"
        >
          {menuOpen ? (
